...
def get_text(text,cleaner_names, cmudict, p_arpabet,g2p):
    ...
    text = ' '.join([get_arpabet(word, cmudict)
                      if random.random() < p_arpabet else word
                      for word in words])
...
def get_phoneme(text,cleaner_names,g2p,tensor=True):
    def text_to_sequence(text):
      sequence = []
      while len(text):
          m = _curly_re.match(text)
    ...
    def get_arpabet(word, g2p, index=0):
      arpabet = g2p(word)
    ...
    text = _clean_text(text, cleaner_names)
    words = re.findall(r'\S*\{.*?\}\S*|\S+', text)
    text = ' '.join([get_arpabet(word, g2p) for word in words])
    if tensor:
      text_norm = torch.LongTensor(text_to_sequence(text))
    else:
...
# def get_phoneme(text,cleaner_names,g2p):
#     def text_to_sequence(text):
#       sequence = []
#       while len(text):
#           m = _curly_re.match(text)
...
def phoneme_tokenizer(g2p, text):
  # g2p = G2p()
  # for text in texts:
      # out = g2p(text)
    return g2p(text)
...
HETERONYMS = set(files_to_list(f'{os.path.dirname(__file__)}/heteronyms'))