...
log_dir = '/nas/users/dahye/kw/tts/ttsxai/logs/probe_tacotron2_energy'
...
def load_npz_files(npz_files):
    ...
    for file in tqdm(npz_files):
        ...
        tokens['target'].append(list(data_dict['energy']))
...
def parallel_load_npz_files(npz_files, num_processes=None):
    ...
    dict_activations = []
    for phonesymbols, energys, articulatory_features, activations in results:
        tokens['source'].append(phonesymbols)
        tokens['target'].append(energys)
